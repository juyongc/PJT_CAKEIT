...
packageName = "package cakeit.server.entity;"
typeMapping = [
        (~/(?i)int/)                      : "Long",
...
//    out.println "// @DynamicUpdate // Apply changed fields only"
    ...
    out.println "public class $className extends AbstractEntity {"
    out.println ""
    fields.each() {
        // output comment
...
//                name    : changeStyle(fileName(col.getName(), false), true),
        ...
        if (col.getName().lastIndexOf("_") != -1 && "_ID".equals(col.getName().substring(col.getName().lastIndexOf("_")))) {
            comm.annos = "\t/** FK setting */\n"
            comm.annos += "\t// @ManyToOne(fetch = FetchType.LAZY)\n"
            comm.annos += "\t// @OneToOne(fetch = FetchType.LAZY)\n"
            comm.annos += "\t// @JoinColumn(referencedColumnName = \"\", name = \"\")"
        }
